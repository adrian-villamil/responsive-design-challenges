...
export const PlanCard = ({
  ...
  return (
    <div className={clsx(
      "w-44 sm:w-55 flex flex-col rounded-2xl overflow-hidden",
      isFree && 'hidden xl:block',
      isTeam && 'hidden lg:block',
      isPopular && 'bg-[#121826] order-last lg:order-0',
      isEnterprise && 'hidden md:block',
    )}>
      <div className="h-52.5 pt-6">
        <p className={clsx(
          "w-fit px-3 rounded-xl mx-auto text-[#3662E3] font-semibold",
          isPopular && 'text-white bg-[#3662E3]'
        )}>{name}</p>
        <h1 className={clsx(
          "mt-1.25 mb-1 text-center text-[64px] font-semibold leading-snug",
      ...
      <div className={clsx(
        'h-18 flex justify-center items-center',
        isPopular && 'bg-[#E9A23B]',
      )}>
        <button className={clsx(
          'pr-5 font-semibold text-[#3662E3] [word-spacing:0.2em] bg-[url("/pricing-table/down-arrow-1.svg")] bg-position-[106%] bg-no-repeat',
...
const FeatureValue = ({ isPopular, value }: FeatureValueProps) => {
  ...
  return (
    <div className={clsx(
      ...
      <p className={clsx(
        "flex justify-center items-center h-12.75 bg-no-repeat bg-center",
